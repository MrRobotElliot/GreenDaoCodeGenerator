...
public class ConvertBridge {
    ...
    public ConvertBridge(Operator operator, String jsonStr, String jsonComment, PsiFile file, Project project, PsiClass targetClass, PsiClass currentClass, String generateClassName) {
    ...
    private void handleVirgoMode(JSONObject json, List<String> fieldList) {
        ...
        FieldsDialog fieldsDialog = new FieldsDialog(operator, generateClassEntity, factory, targetClass, currentClass, file, project, generateClassName);
        fieldsDialog.setSize(1080, 512);
    ...
    private List<FieldEntity> createFields(JSONObject json, Map<String, FieldApiInfo> fieldApiInfoMap, List<String> fieldList, ClassEntity parentClass) {
        ...
        List<FieldEntity> resultList = fieldEntityList.stream().sorted(Comparator.comparing(FieldEntity::getSortNo)).collect(Collectors.toList());
        return resultList;
    }
    ...
    private FieldEntity createField(ClassEntity parentClass, String key, Object type, Map<String, FieldApiInfo> fieldApiInfoMap) {
        ...
        if (fieldName.contains("_")) { // Config.getInstant().isUseSerializedName() &&
            ...
            //fieldName是全部大写
            if (StringUtils.isAcronym(fieldName)) { // Config.getInstant().isUseSerializedName() &&
